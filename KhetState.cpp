...
KhetState::KhetState(KhetState* s, KhetMove* mv) {
  ...
  his = s;
  // apply the move
  // we only pass moves generated by gen()
  // correctness of which has already been validated
  // so making this move is legitimate
  imake(*mv);
...
// makes a KhetMove from its string representation
KhetMove KhetState::makeMove(string algstr) {
  KhetMove mv;
  const char* in = algstr.c_str();
  int fromFile = in[0] - 'a';
  int fromRank = in[1] - '1';
  KhetPiece pc = board[fromFile][fromRank];
  mv.piece = pc;
  mv.fromFile = fromFile;
  mv.fromRank = fromRank;
  mv.fromRot = pc.rot;
  if (in[2] < 'a') {
    // rotation
    if (in[2] == 'L') {
...
long KhetState::gen() 
{
  ...
  for (int rank = 0; rank < 8; rank++) {
    for (int file = 0; file < 10; file++) {
      ...
      int rot1 = (piece.rot + 1) % 4; // rotate left
      int rot2 = (piece.rot + 3) % 4; // rotate right
      //cout << file << " " << rank << endl;
      switch (piece.type) {
        case SPHINX: 
